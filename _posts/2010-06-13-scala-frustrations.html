...
First there's the whole notion that if you name a method ending in :, and you want to call it in operator notation (<span style="font-family: 'Courier New', Courier, monospace;">instance methodname argument</span> rather than <span style="font-family: 'Courier New', Courier, monospace;">instance.methodname(argument)</span>) then you call it by placing it to the left of the object on which you are calling it - so <span style="font-family: 'Courier New', Courier, monospace;">argument methodname: instance</span>. To me that's a humungous special case. At the moment it seems to me that it's been added solely in order to make prepending to Lists more natural, because Scala lists are immutable and so the time taken to append apparently grows linearly whereas the time taken to prepend is constant. I can't help but feel that's an insufficient reason to introduce a massive syntax divergence from the norm. Is there really no other way? Would an immutable List backed by a mutable LinkedList (so appending returns a new view over the same List) really perform so badly when appending? And if it does, could it not be just a case where developers need to be cautious, as with String concatenation in Java?Then there's the operator precedence rules. Scala doesn't have operators, right? Only methods? Wrong. It has a few method names which are special cased to have a different precedence order because they are normally operators in other languages. Nice. So for instance <span style="font-family: 'Courier New', Courier, monospace;">(2 + 2 * 5) == 12</span>, whereas <span style="font-family: 'Courier New', Courier, monospace;">((2).+(2).*(5)) == 20</span>, despite the fact that they are the same methods in the same order on the same objects - Scala just specifies that 2 + 2 * 5 is treated as 2 + (2 * 5). I understand this one, I'm sure it's to align Scala with mathematical convention, but I still disagree. Having a bunch of methods who behave differently in terms of order of evaluation to others based on their name is a huge and confusing special case.
